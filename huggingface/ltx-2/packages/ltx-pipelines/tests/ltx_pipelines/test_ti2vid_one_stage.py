...
def test_img2vid_one_stage(
    ...
    """Run img2vid one-stage pipeline (with image conditioning) and verify output matches expected."""

    output_path = OUTPUT_DIR / "img2vid_one_stage_output.mp4"
    ...
    expected_video, expected_waveform = decode_video_from_file(
        path=ASSETS_DIR / "expected_img2vid_one_stage.mp4", device=device
    )
...
@pytest.mark.e2e
def test_txt2vid_one_stage(
    psnr: Callable[[torch.Tensor, torch.Tensor, float, float], float],
    decode_video_from_file: Callable[[str], tuple[torch.Tensor, torch.Tensor | None]],
) -> None:
    """Run txt2vid one-stage pipeline (no image conditioning) and verify output matches expected."""

    output_path = OUTPUT_DIR / "txt2vid_one_stage_output.mp4"
    ...
    decoded_video, waveform = decode_video_from_file(path=output_path, device=device)
    expected_video, expected_waveform = decode_video_from_file(
        path=ASSETS_DIR / "expected_txt2vid_one_stage.mp4", device=device
    )
