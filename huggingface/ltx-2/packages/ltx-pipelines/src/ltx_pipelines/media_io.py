...
def encode_video(
    video: torch.Tensor | Generator[tuple[torch.Tensor, int], None, None],
    fps: int,
    ...
    def all_tiles(
        ...
        yield first_chunk, total
        yield from tiles_generator

    for video_chunk, _ in tqdm(all_tiles(first_chunk, video), total=total):
