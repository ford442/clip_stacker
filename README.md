...
│   └── media_engine/      # Timeline PCM mix + linear resample
...
Export premix prefers `mixTimelineAudio` when the media-engine module loads. Disable with `?no_media_engine` to force `OfflineAudioContext`. Volume/pan automation curves always use the Web Audio offline graph. Do not put mix/EQ DSP in `canvas-renderer` or FFmpeg `filter_complex`.
