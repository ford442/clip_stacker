...
export const VECTORSCOPE_WGSL = /* wgsl */ `
struct Params {
  binSize: u32,
  _pad0: u32,
  _pad1: u32,
...
const LUMA = vec3f(0.2126, 0.7152, 0.0722);
// Rec.709 chroma denominators: 2(1 - Kb) and 2(1 - Kr).
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  let color = clamp(textureLoad(src, vec2i(gid.xy), 0).rgb, vec3f(0.0), vec3f(1.0));
  let y = dot(color, LUMA);
  let cb = (color.b - y) / CB_SCALE + 0.5;
  let cr = (color.r - y) / CR_SCALE + 0.5;
  let size = params.binSize;
  let maxBin = size - 1u;
  let u = min(u32(max(cb, 0.0) * f32(size)), maxBin);
  let v = min(u32(max(cr, 0.0) * f32(size)), maxBin);
  atomicAdd(&bins[v * size + u], 1u);
}
`;
