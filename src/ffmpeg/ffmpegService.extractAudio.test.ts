...
describe('validateExtractAudioClip', () => {
  ...
  it('returns null for a valid video clip with trim points', () => {
    expect(validateExtractAudioClip(makeClip({ trimStart: 2, trimEnd: 8 }))).toBeNull();
  });

  it('returns null for an audio-only clip', () => {
    expect(
      validateExtractAudioClip(makeClip({ kind: 'audio', file: new File([], 'test.mp3') })),
    ).toBeNull();
  });
  ...
  it('returns an error string when trimStart equals trimEnd (zero duration)', () => {
    const msg = validateExtractAudioClip(makeClip({ trimStart: 5, trimEnd: 5 }));
  ...
  it('returns an error string when trimStart exceeds trimEnd', () => {
    const msg = validateExtractAudioClip(makeClip({ trimStart: 8, trimEnd: 4 }));
    expect(msg).not.toBeNull();
    expect(msg).toContain('zero or negative duration');
  });

  it('returns an error string when clip duration is 0 and trimEnd is NaN', () => {
    const msg = validateExtractAudioClip(makeClip({ duration: 0, trimStart: 0 }));
    expect(msg).not.toBeNull();
    expect(msg).toContain('zero or negative duration');
  });

  it('includes trimStart, trimEnd and duration values in the error message', () => {
    const msg = validateExtractAudioClip(makeClip({ trimStart: 5, trimEnd: 3, duration: 10 }))!;
  ...
  it('shows "end" for trimEnd when trimEnd is NaN', () => {
    const msg = validateExtractAudioClip(makeClip({ duration: 0, trimStart: 0, trimEnd: NaN }))!;
