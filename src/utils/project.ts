...
export function sanitizeClipAdjustments(clip: Clip): void {
  clip.trimStart = Number.isFinite(clip.trimStart)
...
export function serializeProject(
  ...
  return {
    ...
    ...(clipGroups.length > 0
      ? {
          clipGroups: clipGroups.map(
            (group): SerializedClipGroup => ({
              id: group.id,
  ...
  onProgress({
    ...
    progress:
      typeof event.progress === "number"
        ? clampUnitProgress(event.progress)
        : null,
  });
}
...
function countRemoteProjectDownloads(project: Project, clips: Clip[]): number {
  ...
  return project.clips.reduce((count, savedClip) => {
    if (byName.has(savedClip.fileName) || !hasRestorableRemoteMedia(savedClip))
      return count;
    return count + 1;
  }, 0);
}

function buildRemoteDownloadStage(
  index: number,
  total: number,
  fileName: string,
): string {
...
function calculateRemoteDownloadProgress(
  ...
  const completed =
    (clipIndex - 1 + clampUnitProgress(clipProgress)) / clipCount;
...
function inferKind(savedClip: SerializedClip, file: File): ClipKind {
  if (savedClip.kind === "audio" || savedClip.kind === "video")
    return savedClip.kind;
...
export async function serializeProjectWithMedia(
  ...
  for (let i = 0; i < project.clips.length; i++) {
    ...
    if (!clip)
      throw new Error(
        `Could not find source media for clip "${serialized.fileName}".`,
      );
    ...
    if (mediaMode === "embed") {
      ...
      if (!options.mediaClient)
        throw new Error("Remote save requires a storage endpoint.");
      ...
      while (true) {
        ...
        try {
          ...
          throw new Error(
            `Upload aborted at clip ${index}/${total}: ${uploadError.message}`,
          );
        }
      }
    }
...
export async function applyProjectData(
  ...
  for (const savedClip of project.clips) {
    ...
    if (!liveClip && hasRestorableRemoteMedia(savedClip)) {
      ...
      const stage = buildRemoteDownloadStage(
        clipIndex,
        clipCount,
        savedClip.fileName,
      );
      ...
      try {
        const mediaUrl =
          savedClip.sourceMediaDataUrl || savedClip.sourceMediaUrl;
        if (!mediaUrl) throw new Error("No media URL available");
        const blob = await downloadRemoteMedia(
          mediaUrl,
          (clipProgress, indeterminate) => {
            emitRemoteProjectLoadProgress(options.onProgress, {
              stage,
              progress: calculateRemoteDownloadProgress(
                clipIndex,
        ...
        const fileType =
          blob.type || savedClip.fileType || "application/octet-stream";
        ...
        const effectiveDuration = Number.isFinite(restoredDuration)
          ? restoredDuration
          : duration;
        liveClip = {
          id: createClipId(),
        ...
        console.warn(
          `Could not restore media for "${savedClip.fileName}"`,
          error,
        );
        liveClip = undefined;
      }
    ...
    liveClip.trimEnd =
      savedClip.trimEnd == null ? NaN : Number(savedClip.trimEnd);
    liveClip.videoFadeIn = Number(
      savedClip.videoFadeIn ?? liveClip.videoFadeIn,
    );
    liveClip.videoFadeOut = Number(
      savedClip.videoFadeOut ?? liveClip.videoFadeOut,
    );
    liveClip.audioFadeIn = Number(
      savedClip.audioFadeIn ?? liveClip.audioFadeIn,
    );
    liveClip.audioFadeOut = Number(
      savedClip.audioFadeOut ?? liveClip.audioFadeOut,
    );
    liveClip.groupId = savedClip.groupId;
    liveClip.groupVariant = savedClip.groupVariant;
    if (savedClip.remoteAudioUrl)
      liveClip.remoteAudioUrl = savedClip.remoteAudioUrl;
    if (savedClip.rifeProcessed) {
      liveClip.rifeProcessed = savedClip.rifeProcessed;
      if (savedClip.rifeMultiplier != null)
        liveClip.rifeMultiplier = savedClip.rifeMultiplier;
      if (savedClip.originalFps != null)
        liveClip.originalFps = savedClip.originalFps;
      if (savedClip.processedFps != null)
        liveClip.processedFps = savedClip.processedFps;
      if (savedClip.rifeMode) liveClip.rifeMode = savedClip.rifeMode;
    }
    if (savedClip.layerIndex != null)
      liveClip.layerIndex = Number(savedClip.layerIndex);
...
export class ContaboStorageManagerClient {
  ...
  async list(): Promise<{ name: string; modified: number }[]> {
    ...
    if (!response.ok)
      throw new Error(`Remote list failed (${response.status})`);
    const result = (await response.json()) as {
      projects: { name: string; modified: number }[];
    };
    ...
    return await new Promise<string>((resolve, reject) => {
      ...
      request.onload = () => {
        ...
        try {
          ...
          reject(
            new Error(
              `Media upload failed (invalid JSON response: ${(error as Error).message})`,
            ),
          );
        }
      };
