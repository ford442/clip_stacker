...
function createRemoteProject(fileName: string, sourceMediaUrl = `https://example.com/media/${fileName}`): Project {
  return {
    clips: [
      {
        id: `saved-${fileName}`,
...
describe('remote project load progress', () => {
  ...
  it('emits manifest and completion progress during remote project load', async () => {
    ...
    await loadRemoteProject(client, 'demo-project', [existingClip], { onProgress });
  ...
  it('emits smooth download progress while restoring remote clips', async () => {
    const fetchMock = vi.fn().mockResolvedValue(createStreamingResponse(['ab', 'cd']));
    vi.stubGlobal('fetch', fetchMock);
    const onProgress = vi.fn();

    const result = await applyProjectData(createRemoteProject('remote.mp4'), [], { onProgress });

    expect(fetchMock).toHaveBeenCalledWith('https://example.com/media/remote.mp4');
  ...
  it('falls back to indeterminate clip progress when streaming details are unavailable', async () => {
    ...
    await applyProjectData(createRemoteProject('fallback.mp4'), [], { onProgress });

    const downloadEvents = onProgress.mock.calls
      .map(([event]) => event)
      .filter((event) => event.stage === 'Downloading clip 1 of 1: fallback.mp4');
