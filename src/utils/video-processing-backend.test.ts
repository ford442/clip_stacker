...
describe('HuggingFaceRifeBackend', () => {
  ...
  it('processClip delegates to processClipWithRIFE', async () => {
    ...
    const result = await backend.processClip(fakeBlob, { multiplier: 2, mode: 'interpolation' });

    expect(mockProcessClipWithRIFE).toHaveBeenCalledOnce();
    const [calledBlob, calledMult, calledMode] = mockProcessClipWithRIFE.mock.calls[0];
  ...
  it('processClip translates progress events', async () => {
    const fakeBlob = new Blob([], { type: 'video/mp4' });
    mockProcessClipWithRIFE.mockImplementationOnce(async (_blob, _mult, _mode, onProgress) => {
    ...
    await backend.processClip(fakeBlob, { multiplier: 2, mode: 'interpolation' }, (e) => {
...
describe('SelfHostedRifeBackend', () => {
  ...
  describe('processClip', () => {
    it('posts to /predict and returns blob from url field', async () => {
      ...
      const fetchMock = vi.fn()
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ data: [{ url: 'https://example.com/result.mp4' }] }),
        })
      ...
      const result = await backend.processClip(inputBlob, { multiplier: 2, mode: 'interpolation' });
    ...
    it('throws when /predict returns a non-2xx status', async () => {
      ...
      await expect(
        backend.processClip(inputBlob, { multiplier: 2, mode: 'interpolation' }),
    ...
    it('appends boomerang=true for boomerang mode', async () => {
      ...
      const inputBlob = new Blob(['input'], { type: 'video/mp4' });
      await backend.processClip(inputBlob, { multiplier: 4, mode: 'boomerang' });
